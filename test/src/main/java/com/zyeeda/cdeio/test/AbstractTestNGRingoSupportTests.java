 ...
 *
 */
@ContextConfiguration("classpath:spring/cdeio/test-application-context.xml")
public abstract class AbstractTestNGRingoSupportTests extends AbstractTestNGSpringContextTests {
    ...
    protected List<String> getRingoSystemModules() {
        ...
        systemModules.add("cdeio-test-modules");
    ...
    protected void configRingo(RingoConfig config) {
        ...
        try {
            ...
            for (Iterator<Repository> it = config.getRepositories().iterator(); it.hasNext(); ) {
                ...
                if (!repo.exists() && "cdeio-test-modules".equals(repo.getName())) {
                    removed = true;
                    it.remove();
                }
            }
            if (removed) {
                config.addModuleRepository(config.resolveRepository(getClassLoader().getResource("cdeio-test-modules").getFile(), false));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void startRingoTests() throws Exception {
        ...
        try {
            ...
            engine.invoke("cdeio/test/test-runner", "run", cx, engine, modules.toArray(), jdbc);
        } finally {
            Context.exit();
        }
    }
