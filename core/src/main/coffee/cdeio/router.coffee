...
{json, html, notFound, internalServerError} = require 'cdeio/response'
...
{createService} = require 'cdeio/scaffold/service'
baseService = require('cdeio/service').createService()
{createConverter} = require 'cdeio/scaffold/converter'
{createValidator} = require 'cdeio/validation/validator'
{createValidationContext} = require 'cdeio/validation/validation-context'

{createManager} = require 'cdeio/manager'

# processService = require 'cdeio/scaffold/process-service'

{Context} = com.zyeeda.cdeio.web.SpringAwareJsgiServlet
{Create, Update} = com.zyeeda.cdeio.validation.group

timeFormat = new java.text.SimpleDateFormat 'yyyy-MM-dd HH:mm:ss'

log = getLogger module.id
entityMetaResovler = Context.getInstance(module).getBeanByClass(com.zyeeda.cdeio.web.scaffold.EntityMetaResolver)
...
getAccountService = ->
    entityClass = ClassUtils.forName 'com.zyeeda.cdeio.commons.organization.entity.Account'
...
defaultHandlers = (path, options) ->
    o =
        list: (options, service, entityMeta, request, taskType) ->
            ...
            filters = cdeio.extractFilterInfo request.params
            ...
            orderInfo = cdeio.extractOrderInfo request.params
            ...
            o = cdeio.generateListResult result.results, config.currentPage, config.maxResults, result.recordCount, result.pageCount
    ...
    return o if options.disableAuthz is true or cdeio.disableAuthz is true
