{createRouter, getJsonFilter} = require 'cdeio/router'
{mark} = require 'cdeio/mark'
{json} = require 'cdeio/response'
{generateForms} = require 'cdeio/scaffold/form-generator'
{requireScaffoldConfig} = require 'cdeio/scaffold/router'
...
metas = entityMetaResolver.resolveScaffoldEntities cdeio.entityPackages

mountExtraRoutes = (r, meta, options) ->
    router.get('configuration/feature', (request) ->
        feature = options.feature or {}
        # 默认 style 为 grid ，cdeio 所支持的类型在 cdeio/scaffold/scaffold-feature-loader 中进行定义
    ...
    r.get('configuration/operators/:taskType', (request, taskType) ->
        ...
        if taskType is 'none'
            ...
            operators = objects.extend {}, cdeio.defaultOperators, ops

        for k, v of operators
            if operators[k] is false
                delete operators[k]
            else
                operators[k] = objects.extend {}, cdeio.defaultOperators[k], operators[k] if k of cdeio.defaultOperators
...
taskQuery = (createQuery, request, process, resolver, noExtra) ->
    configs = cdeio.extractPaginationInfo request.params
    orders = cdeio.extractOrderInfo request.params
    ...
    configs || (configs = {})
    o = cdeio.generateListResult results, configs.currentPage, configs.maxResults, result.recordCount, result.pageCount
    json o, filter

processQuery = (process, request, resolver) ->
    configs = cdeio.extractPaginationInfo(request.params) or {}
    orders = cdeio.extractOrderInfo request.params
    ...
    o = cdeio.generateListResult result.items, configs.currentPage, configs.maxResults, result.recordCount, result.pageCount
    json o, filter
...
router.get '/configuration/forms/:taskId', mark('process').mark('beans', EntityMetaResolver).on (process, resolver, request, taskId) ->
    ...
    if isHistoric
        ...
        form.fieldGroups['task-info'].push type: 'feature', path: 'cdeio:task-grid', options:
