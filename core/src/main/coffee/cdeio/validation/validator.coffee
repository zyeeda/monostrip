    ...
    for f in fields
        ...
        for a in newAnnos
            ...
            if groups.length == 0 or ArrayUtils.contains groups, group
                ...
                else if a instanceof com.zyeeda.cdeio.validation.constraint.NullableSize
                ...
                else if a instanceof com.zyeeda.cdeio.validation.constraint.Matches
                    rules[name].equalTo = a.target()

        classAnnos = entityClass.annotations
        for anno in classAnnos
            if anno instanceof com.zyeeda.cdeio.validation.constraint.Matches
                name = anno.target()
                rules[name] = {} unless rules[name]
